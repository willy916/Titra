...
onMounted(async () => {
  try {
    await transformerStore.fetchDashboardStats()
  } catch (error) {
  ...
  <div class="min-h-screen bg-background pb-24">
    ...
    <div class="bg-primary text-white p-6 rounded-b-3xl">
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 class="text-white text-2xl">{{ transformerStore.dashboardStats?.transformateur.name || user.name }}</h1>
      ...
      <div class="grid grid-cols-2 gap-3">
        <Card class="bg-white/10 border-white/20 backdrop-blur p-4">
          ...
          <p class="text-white text-xl">{{ (transformerStore.dashboardStats?.wallet.availableBalance || 0).toLocaleString() }} F</p>
        </Card>
        <Card class="bg-white/10 border-white/20 backdrop-blur p-4">
          ...
          <p class="text-white text-xl">{{ (transformerStore.dashboardStats?.wallet.pendingBalance || 0).toLocaleString() }} F</p>
        </Card>
      </div>
    </div>
    ...
    <div class="p-6 space-y-6">
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-3">
          <Button @click="emit('navigate', 'marketplace', { sellerType: 'PRODUCTEUR' })" class="h-auto flex-col gap-2 py-4 bg-secondary"><ShoppingBag class="w-6 h-6" /><span class="text-sm text-[14px]">Acheter matières <br/>premières</span></Button>
      ...
      <Card class="p-4">
        ...
        <div class="grid grid-cols-3 gap-4">
          <template v-if="transformerStore.isLoading && !transformerStore.dashboardStats">
          ...
          <template v-else>
            <div class="text-center"><p class="text-muted-foreground text-sm">Achats</p><p class="text-2xl text-primary">{{ transformerStore.dashboardStats?.stats.purchaseCount || 0 }}</p></div>
            <div class="text-center"><p class="text-muted-foreground text-sm">Ventes</p><p class="text-2xl text-secondary">{{ transformerStore.dashboardStats?.stats.saleCount || 0 }}</p></div>
            <div class="text-center"><p class="text-muted-foreground text-sm">Revenus</p><p class="text-lg text-primary">{{ ((transformerStore.dashboardStats?.stats.totalRevenue || 0) / 1000).toFixed(0) }}k</p></div>
          </template>
        </div>
      </Card>
      ...
      <div>
        ...
        <div class="space-y-2">
          ...
          <template v-else>
            <Card v-for="purchase in transformerStore.dashboardStats?.recentPurchases" :key="purchase.id" class="p-4" @click="emit('navigate', 'order-details', { orderId: purchase.id })">
              <div class="flex items-start justify-between">
                <div class="flex gap-3 flex-1">
                  ...
                  <div class="flex-1">
                    ...
                    <p class="text-sm text-muted-foreground">{{ purchase.subtitle }}</p>
                ...
                <div class="text-right">
                  ...
                  <Badge variant="outline" class="text-[10px] h-4 px-1 mt-1">{{ purchase.status }}</Badge>
                </div>
              </div>
            </Card>
          </template>
      ...
      <div>
        ...
        <div class="space-y-2">
          ...
          <template v-else>
            <Card v-for="sale in transformerStore.dashboardStats?.recentSales" :key="sale.id" class="p-4" @click="emit('navigate', 'order-details', { orderId: sale.id })">
              <div class="flex items-start justify-between">
                <div class="flex gap-3 flex-1">
                  ...
                  <div class="flex-1">
                    <p class="font-medium">{{ sale.title }}</p>
                    <p class="text-sm text-muted-foreground">{{ sale.subtitle }}</p>
                    <Badge :variant="sale.status === 'DELIVERED' ? 'default' : 'secondary'" class="mt-1 text-xs">{{ sale.status }}</Badge>
                  </div>
                </div>
