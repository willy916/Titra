...
<template>
  <div class="min-h-screen bg-background p-4 pb-8">
    <div class="max-w-md mx-auto">
      <div class="mb-8"><h1 class="text-primary mb-2">Profil Interprofession</h1><p class="text-muted-foreground">Étape {{ step }} sur 4</p><Progress :value="progress" class="mt-4" /></div>
      ...
      <div v-else-if="step === 2" class="space-y-6">
        ...
        <div class="space-y-2"><Label>Ou saisissez une filière personnalisée</Label><Input v-model="formData.customFiliere" placeholder="Ex: Riz, Mangue, Karité..." @input="formData.filiere = ''" /><p class="text-xs text-muted-foreground">Laissez vide si vous avez sélectionné une filière ci-dessus</p></div>
        <div class="space-y-2"><Label>Nombre de fédérations membres</Label><div class="relative"><Users class="absolute left-3 top-3 h-5 w-5 text-muted-foreground" /><Input v-model="formData.membersCount" type="number" placeholder="Ex: 5 fédérations" class="pl-10" /></div></div>
        <div class="space-y-2">
          <Label>Filières secondaires (optionnel)</Label><p class="text-sm text-muted-foreground">Ajoutez d'autres filières dans lesquelles votre interprofession est active</p>
      ...
      <div v-else-if="step === 3" class="space-y-6">
        <h2 class="text-primary">Affiliation à une instance nationale (optionnel)</h2>
        <InstitutionalAffiliationStep user-role="interprofession" :selected-affiliation="formData.affiliationId" :can-be-independent="true" @select="handleAffiliationSelect" />
      ...
      <div v-else class="space-y-6">
        <TeamMembersStep :members="formData.teamMembers" structure-type="interprofession" @update:members="formData.teamMembers = $event" />
