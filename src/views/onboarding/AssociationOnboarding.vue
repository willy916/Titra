...
const isStep1Valid = computed(() => formData.value.associationName && formData.value.registrationNumber && formData.value.presidentName && formData.value.location)
...
<template>
  <div class="min-h-screen bg-background p-4 pb-8">
    <div class="max-w-md mx-auto">
      <div class="mb-8"><h1 class="text-primary mb-2">Profil Association</h1><p class="text-muted-foreground">Étape {{ step }} sur 4</p><Progress :value="progress" class="mt-4" /></div>

      <div v-if="step === 1" class="space-y-6">
        <h2 class="text-primary">Informations de l'association</h2>
        <div class="space-y-2"><Label>Nom de l'association</Label><div class="relative"><Building2 class="absolute left-3 top-3 h-5 w-5 text-muted-foreground" /><Input v-model="formData.associationName" placeholder="Ex: Association des Producteurs Bio" class="pl-10" /></div></div>
        <div class="space-y-2"><Label>Numéro d'enregistrement</Label><Input v-model="formData.registrationNumber" placeholder="Ex: CI-ASSO-2020-001" /></div>
        <div class="space-y-2"><Label>RCCM</Label><Input v-model="formData.rccm" placeholder="Ex: RCCM 2020/001" /></div>
        <div class="space-y-2"><Label>NCC</Label><Input v-model="formData.ncc" placeholder="Ex: NCC 2020/001" /></div>
      ...
      <div v-else-if="step === 2" class="space-y-6">
        ...
        <div class="space-y-2">
          <Label>Filières secondaires (optionnel)</Label><p class="text-sm text-muted-foreground">Ajoutez d'autres filières dans lesquelles votre association est active</p>
      ...
      <div v-else-if="step === 3" class="space-y-6">
        ...
        <InstitutionalAffiliationStep user-role="association" :selected-affiliation="formData.affiliationId" :can-be-independent="true" @select="handleAffiliationSelect" />
