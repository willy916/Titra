  ...
  <div class="min-h-screen bg-background pb-20 lg:pb-0">
    ...
    <div class="p-4 lg:p-0 space-y-6">
      ...
      <div class="flex gap-2 border-b overflow-x-auto">
        <button @click="activeTab = 'books'" :class="['flex items-center gap-1 px-4 py-2 text-sm font-medium border-b-2 -mb-px whitespace-nowrap', activeTab === 'books' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground']">
          <BookOpen class="w-4 h-4" />Livres
        </button>
        <button @click="activeTab = 'videos'" :class="['flex items-center gap-1 px-4 py-2 text-sm font-medium border-b-2 -mb-px whitespace-nowrap', activeTab === 'videos' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground']">
          <Video class="w-4 h-4" />Vidéos
        </button>
        <button @click="activeTab = 'youtube'" :class="['flex items-center gap-1 px-4 py-2 text-sm font-medium border-b-2 -mb-px whitespace-nowrap', activeTab === 'youtube' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground']">
          <Youtube class="w-4 h-4" />YouTube
        </button>
        <button @click="activeTab = 'trainers'" :class="['flex items-center gap-1 px-4 py-2 text-sm font-medium border-b-2 -mb-px whitespace-nowrap', activeTab === 'trainers' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground']">
        ...
        <Card v-for="trainer in trainings.trainers" :key="trainer.id" class="p-6 hover:shadow-lg transition-shadow">
          <div class="flex items-start gap-4">
            <div class="relative">
              ...
              <div v-if="trainer.online" class="absolute bottom-0 right-0 w-4 h-4 bg-success rounded-full border-2 border-white"></div>
            </div>
            <div class="flex-1">
